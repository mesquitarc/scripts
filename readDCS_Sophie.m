function [int,corrs,taus,marks,varargout] = readDCS(filenm,cutusingdefinednumber,datalength,Ns,Nd,plotint)
...
while (morefiles)
    ...
    if exist(fname)==2
        ...
        tmp = fscanf(fi,'%s',[13]);
        inttmp = fscanf(fi,'%g %g %g %g %g %g %g %g %g',[8 1]);
        clear tmp
        tmp = fscanf(fi,'%s',[28]);
        data = fscanf(fi,'%g %g %g %g %g %g %g %g %g',[9 256]);
        marktmp = fscanf(fi,'%g');
        fclose(fi);
        
        % Cutting data
        if cutusingdefinednumber==1
            int(numfiles,:)=inttmp(1,2:Nd+1);
...
if plotint
    figure;
    subplot(2,1,1)
    plot(1:size(int,1),int(:,1:4));
    title(['DCS Intensity at all APDs'])
    legend('1','2','3','4')
    xlabel('Frame'); ylabel('Amplitude (kHz)')
    for k=1:length(marks)
        line([marks(k) marks(k)],[min(min(int(:,1:4))) max(max(int(:,1:4)))],'Color',[0 0 0]);
        text(marks(k), max(max(int(:,1:4))), num2str(k), 'Color', 'b');
    end
    if size(int,2)>4
        subplot(2,1,2)
        plot(1:size(int,1),int(:,5:8));
        legend('5','6','7','8')
        xlabel('Frame'); ylabel('Amplitude (kHz)')
        for k=1:length(marks)
            line([marks(k) marks(k)],[min(min(int(:,5:8))) max(max(int(:,5:8)))],'Color',[0 0 0]);
            text(marks(k), max(max(int(:,5:8))), num2str(k), 'Color', 'b');
        end
    end
end
