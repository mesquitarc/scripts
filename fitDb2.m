function [Betasave,Dbfit,Curvefit,varargout] = fitDb2(intensitydata,corrs,taus,fsdlist,Ns,Nd,fitbeta,cutusingdefinednumber,datalength,Marksflow)
%
    ...
    for snum=1:Ns
        for i=1:Nd
            ...
            if fitbeta==1
                ...
                options = optimset('TolFun',1E-6,'TolX',1E-6,'MaxFunEvals',2E4,'MaxIter',2E4,'display','iter',...
                    'Diagnostics','off','NonlEqnAlgorithm','lm','LargeScale','off');
                warning('off','MATLAB:declareGlobalBeforeUse')
                taufit = squeeze(taus(rk,1:tmpf(i))); global taufit
                corrfit = squeeze(corrstmp(:)); global corrfit
                sdfit = fsdlist(snum,i); global sdfit
                global muspo
                global muao
                global ko
                global ze
                global Dbr0
                tmpffit = tmpf(i); global tmpffit
                global mux0
                global corrs0
                Beta(i)=lsqnonlin('curvefit2',0.5,0.4,.8);
            elseif fitbeta==2
                Beta(i)=0.5;
            else
                Beta(i)=corrs0;
